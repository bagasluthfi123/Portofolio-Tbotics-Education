...
// ==========================================
// KOMPONEN BACKGROUND: Matahari Neon & Partikel (VERSI NERF EKSTRIM CSS MURNI)
// ==========================================
const NeonCosmicBackground = () => {
  const [particles, setParticles] = useState([]);

  // Generate partikel di dalam useEffect
  useEffect(() => {
    // Partikel dikurangi jadi 6 agar sangat ringan
    const generatedParticles = [...Array(6)].map((_, i) => ({
      id: i,
      size: Math.random() * 4 + 2, 
      top: `${Math.random() * 100}%`,
      left: `${Math.random() * 100}%`,
      animDuration: `${Math.random() * 8 + 8}s`, // Tambah 's' untuk CSS (detik)
      animDelay: `${Math.random() * 3}s`, // Tambah 's' untuk CSS (detik)
    }));
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 flex items-center justify-center">
      ...
      <style>
        {`
          @keyframes float-program-particle {
            0%, 100% { transform: translateY(0); opacity: 0; }
            50% { transform: translateY(-50px); opacity: 0.8; }
          }
          .animate-program-particle {
            animation-name: float-program-particle;
      ...
      {/* 1. Aura Gelap Terluar (Radial gradient statis, BUKAN blur) */}
      <div className="absolute w-[400px] md:w-[700px] h-[400px] md:h-[700px] bg-[radial-gradient(circle,rgba(30,58,138,0.25)_0%,transparent_70%)]" />

      {/* 2. Inti Matahari Neon (Radial gradient + CSS animate-pulse bawaan Tailwind) */}
      <div 
        className="absolute w-[200px] md:w-[400px] h-[200px] md:h-[400px] bg-[radial-gradient(circle,rgba(6,182,212,0.4)_0%,transparent_70%)] animate-pulse"
        style={{ animationDuration: '4s' }}
      />
      
      {/* 3. Pusat Inti Paling Terang (Radial gradient statis) */}
      <div className="absolute w-[100px] md:w-[150px] h-[100px] md:h-[150px] bg-[radial-gradient(circle,rgba(255,255,255,0.2)_0%,transparent_60%)]" />

      {/* 4. Partikel Bintang (Div biasa + animasi CSS murni, Tanpa Framer Motion!) */}
      {particles.map((p) => (
        <div
          key={p.id}
          className="absolute bg-cyan-300 rounded-full shadow-[0_0_8px_rgba(34,211,238,0.8)] animate-program-particle"
          style={{
            width: p.size,
            height: p.size,
            top: p.top,
